...
class OrtTensor : public ITensor {
public:
  ...
  void SetBufferLocation(DataLocation location) override
  {
    CHECK_STATE_THROW(location != DataLocation::UNKOWN,
                      "[OrtTensor] `SetBufferLocation` Got invalid location: UNKOWN !");
  }
  ...
  void ToLocation(DataLocation location) override
  {
    assert(current_location_ != DataLocation::UNKOWN);
    CHECK_STATE_THROW(location != DataLocation::UNKOWN,
                      "[OrtTensor] `ToLocation` Got invalid location: UNKOWN !");
  }
  ...
  {
    CHECK_STATE_THROW(tensor != nullptr, "[OrtTensor] `ZeroCopy` Got invalid tensor: nullptr !");
    auto location = tensor->GetBufferLocation();
    CHECK_STATE_THROW(location != DataLocation::UNKOWN,
                      "[OrtTensor] `ZeroCopy` Got invalid tensor location: UNKOWN !");
    ...
    CHECK_STATE_THROW(raw_ptr != nullptr,
                      "[OrtTensor] `ZeroCopy` Got invalid tensor raw_ptr: nullptr !");
  ...
  {
    CHECK_STATE_THROW(tensor != nullptr, "[OrtTensor] `DeepCopy` Got invalid tensor: nullptr !");
    auto location = tensor->GetBufferLocation();
    CHECK_STATE_THROW(location != DataLocation::UNKOWN,
                      "[OrtTensor] `DeepCopy` Got invalid tensor location: UNKOWN !");
    if (location == DataLocation::DEVICE)
    {
      tensor->ToLocation(DataLocation::HOST);
    }
    auto raw_ptr = tensor->RawPtr();
    CHECK_STATE_THROW(raw_ptr != nullptr,
                      "[OrtTensor] `DeepCopy` Got invalid tensor raw_ptr: nullptr !");
  ...
  void SetShape(const std::vector<size_t> &shape) override
  {
    ...
    CHECK_STATE_THROW(this_shape_byte_size <= GetBufferMaxByteSize(),
                      "[OrtTensor] `SetShape` Got invalid shape: exceeds max byte size !");
